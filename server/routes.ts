...
import { db } from "./db";
import { users, posts, reports, loginSchema, signupSchema, insertPostSchema, insertReportSchema } from "../shared/schema";
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get("/api/posts", async (req: Request, res: Response) => {
    try {
      ...
      return res.json(allPosts.map(p => ({
        ...p,
  ...
  app.get("/api/posts/user/:userId", async (req: Request, res: Response) => {
    try {
      ...
      return res.json(userPosts.map(p => ({
        ...p,
        createdAt: new Date(p.createdAt).getTime(),
        authorDisplayName: p.isAnonymous ? undefined : p.authorDisplayName,
      })));
