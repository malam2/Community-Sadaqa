...
export async function fetchUserPosts(userId: string): Promise<Post[]> {
  const response = await fetch(new URL(`/api/posts/user/${userId}`, getApiUrl()).toString());
  if (!response.ok) {
...
export async function updatePost(id: string, userId: string, updates: Partial<Post>): Promise<Post> {
  const response = await fetch(new URL(`/api/posts/${id}`, getApiUrl()).toString(), {
    method: "PATCH",
    headers: { "Content-Type": "application/json" },
