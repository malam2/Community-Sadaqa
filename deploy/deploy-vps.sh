...
if [ ! -f .env.production ]; then
    ...
    echo -e "${YELLOW}⚠️  Please edit .env.production with your credentials before proceeding${NC}"
    exit 1
fi

# Check if Caddyfile is configured
if grep -q "YOUR_DOMAIN.com" Caddyfile; then
    echo -e "${YELLOW}⚠️  Please update Caddyfile with your actual domain${NC}"
...
docker compose -f docker-compose.prod.yml exec server npm run db:push || echo "Run db:push locally with production DATABASE_URL"
